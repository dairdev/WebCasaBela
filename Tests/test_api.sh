...
show_help() {
    ...
    echo -e "${CYAN}========================================${NC}"
    echo ""
    echo -e "${YELLOW}Uso:${NC} $0 [comando] [parámetros]"
    echo ""
    ...
    echo -e "  ${GREEN}user <id>${NC}                       - Obtener usuario específico"
    ...
    echo -e "  ${GREEN}property <id>${NC}                   - Obtener propiedad específica"
    ...
    echo -e "  ${GREEN}property-stats${NC}                  - Obtener estadísticas de propiedades"
    echo -e "  ${GREEN}me${NC}                              - Obtener información del usuario actual"
    ...
    echo -e "  ${BLUE}JWT_TOKEN${NC}     - Token JWT (se llena automáticamente con login)"
...
# Función para hacer login y obtener JWT token
do_login() {
    echo -e "${YELLOW}🔐 Haciendo login...${LOGIN_EMAIL}/${LOGIN_PASSWORD} ${NC}"
    ...
    if [ $status_code -eq 200 ]; then
        ...
        if command -v jq &> /dev/null; then
            ...
            # Extracción manual del token si jq no está disponible
            export JWT_TOKEN=$(echo $body | grep -o '"token":"[^"]*' | grep -o '[^"]*$')
        fi

        echo -e "${YELLOW}Exported Token in JWT_TOKEN: ${JWT_TOKEN}"
...
check_token() {
    ...
    if [ -z "$JWT_TOKEN" ]; then
        echo -e "${RED}❌ No hay JWT token. Ejecuta primero: $0 login${NC}"
        exit 1
    fi
}
...
get_user() {
    local user_id=$1
    if [ -z "$user_id" ]; then
        echo -e "${RED}❌ Error: Se requiere ID de usuario${NC}"
...
get_property() {
    ...
    if [ -z "$property_id" ]; then
        echo -e "${RED}❌ Error: Se requiere ID de propiedad${NC}"
...
get_property_stats() {
    make_auth_request "GET" "/api/properties/stats" "" "ESTADÍSTICAS DE PROPIEDADES"
}

get_me() {
    make_auth_request "GET" "/api/auth/me" "" "INFORMACIÓN USUARIO ACTUAL"
}

check_status() {
    echo -e "${YELLOW}📊 Verificando estado de la API...${NC}"
...
test_all() {
    echo -e "${CYAN}🚀 Ejecutando todas las pruebas...${NC}\n"
...
# Verificar dependencias y ejecutar función principal
check_dependencies
main "$@"
